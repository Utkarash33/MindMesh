...
const Interview = () => {
  ...
  const [provideFeedback, setProvideFeedback] = useState<string>("gg\ngg");
  const [isListening, setIsListening] = useState<boolean>(false); // Added state for listening toggle
  const { transcript, resetTranscript, browserSupportsSpeechRecognition } =
    useSpeechRecognition();

  const feedbackarray = provideFeedback && provideFeedback.split("\n");
    ...
    <div className="max-w-[80%] md:mt-[50px] sm:mt-[-60px] w-full h-[83vh] mx-auto text-center flex flex-col justify-center">
      {feedback ? (
        <div className="bg-[#849fd2] text-start p-6 w-[90%] md:w-[70%] mx-auto rounded-xl shadow-md space-x-4">
          <h2 className="text-white text-sm font-bold md:text-lg">Feedback</h2>
          ...
          <div className="h-[100%]">
            ...
            <div className="flex items-center md:flex-row mt-[-10px] flex-col h-[80%]">
              {isListening ? (
                ...
                <textarea
                  className="rounded-lg py-4 h-[80%] w-[90%] h-32 px-3 mt-4 mr-2 text-start"
...
//     // console.log(track)
//     const startListening=(a:React.MouseEvent<HTMLButtonElement>)=>{
//         a.preventDefault()
//         SpeechRecognition.startListening({ continuous: true,language:"en-IN" })
//     }
...
//         }
//     };
//     const stopListen=(a:React.MouseEvent<HTMLButtonElement>)=>{
//         a.preventDefault()
//         SpeechRecognition.stopListening()
//     }
